// Generated by BUCKLESCRIPT VERSION 5.0.0, PLEASE EDIT WITH CARE
'use strict';

var List = require("bs-platform/lib/js/list.js");
var Block = require("bs-platform/lib/js/block.js");
var Curry = require("bs-platform/lib/js/curry.js");
var Js_null = require("bs-platform/lib/js/js_null.js");
var Caml_option = require("bs-platform/lib/js/caml_option.js");
var Json_decode = require("@glennsl/bs-json/lib/js/src/Json_decode.bs.js");
var Json_encode = require("@glennsl/bs-json/lib/js/src/Json_encode.bs.js");
var Caml_exceptions = require("bs-platform/lib/js/caml_exceptions.js");

var NotImplemented = Caml_exceptions.create("Data-Record.NotImplemented");

function op_of_payload(payload) {
  if (typeof payload === "number") {
    return 11;
  } else {
    switch (payload.tag | 0) {
      case 0 : 
          return 0;
      case 1 : 
          return 1;
      case 2 : 
          return 2;
      case 3 : 
          return 10;
      
    }
  }
}

function t_of_payload(payload) {
  if (typeof payload === "number" || payload.tag) {
    return undefined;
  } else {
    return "READY";
  }
}

function user(json) {
  return /* record */Block.record([
            "id",
            "username",
            "discriminator",
            "avatar",
            "bot",
            "mfa_enabled",
            "locale",
            "verified",
            "email",
            "flags",
            "premium_type"
          ], [
            Json_decode.field("id", Json_decode.string, json),
            Json_decode.field("username", Json_decode.string, json),
            Json_decode.field("discriminator", Json_decode.string, json),
            Json_decode.field("avatar", (function (param) {
                    return Json_decode.nullable(Json_decode.string, param);
                  }), json),
            Json_decode.field("bot", (function (param) {
                    return Json_decode.optional(Json_decode.bool, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("mfa_enabled", Json_decode.bool, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("locale", Json_decode.string, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("verified", Json_decode.bool, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("email", Json_decode.string, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("flags", Json_decode.$$int, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("premium_type", Json_decode.$$int, param);
                  }), json)
          ]);
}

var User = /* module */Block.localModule(["user"], [user]);

function unavailableGuild(json) {
  return /* record */Block.record([
            "id",
            "unavailable"
          ], [
            Json_decode.field("id", Json_decode.string, json),
            Json_decode.field("unavailable", Json_decode.bool, json)
          ]);
}

var Guild = /* module */Block.localModule(["unavailableGuild"], [unavailableGuild]);

function ready(json) {
  return /* Event */Block.variant("Event", 0, [/* Ready */Block.simpleVariant("Ready", [/* record */Block.record([
                    "v",
                    "user",
                    "private_channels",
                    "guilds",
                    "session_id",
                    "trace",
                    "shard"
                  ], [
                    Json_decode.field("v", Json_decode.$$int, json),
                    Json_decode.field("user", user, json),
                    Json_decode.field("private_channels", (function (param) {
                            return Json_decode.array((function (param) {
                                          return /* () */0;
                                        }), param);
                          }), json),
                    Json_decode.field("guilds", (function (param) {
                            return Json_decode.array(unavailableGuild, param);
                          }), json),
                    Json_decode.field("session_id", Json_decode.string, json),
                    Json_decode.field("_trace", (function (param) {
                            return Json_decode.array(Json_decode.string, param);
                          }), json),
                    Json_decode.optional((function (param) {
                            return Json_decode.field("shard", (function (param) {
                                          return Json_decode.array(Json_decode.$$int, param);
                                        }), param);
                          }), json)
                  ])])]);
}

function hello(json) {
  return /* Hello */Block.variant("Hello", 3, [/* record */Block.record([
                "heartbeat_interval",
                "trace"
              ], [
                Json_decode.field("heartbeat_interval", Json_decode.$$int, json),
                Json_decode.field("_trace", (function (param) {
                        return Json_decode.array(Json_decode.string, param);
                      }), json)
              ])]);
}

function data(json) {
  var match = Json_decode.field("op", Json_decode.$$int, json);
  var tmp;
  switch (match) {
    case 0 : 
        var match$1 = Json_decode.field("t", Json_decode.string, json);
        if (match$1 === "READY") {
          tmp = ready;
        } else {
          throw NotImplemented;
        }
        break;
    case 1 : 
    case 2 : 
    case 3 : 
    case 4 : 
    case 5 : 
    case 6 : 
    case 7 : 
    case 8 : 
    case 9 : 
        throw NotImplemented;
    case 10 : 
        tmp = hello;
        break;
    case 11 : 
        tmp = (function (param) {
            return /* Ack */0;
          });
        break;
    default:
      throw NotImplemented;
  }
  return /* record */Block.record([
            "op",
            "d",
            "s",
            "t"
          ], [
            Json_decode.field("op", Json_decode.$$int, json),
            Json_decode.field("d", tmp, json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("s", Json_decode.$$int, param);
                  }), json),
            Json_decode.optional((function (param) {
                    return Json_decode.field("t", Json_decode.string, param);
                  }), json)
          ]);
}

var Decode = /* module */Block.localModule([
    "User",
    "Guild",
    "ready",
    "hello",
    "data"
  ], [
    User,
    Guild,
    ready,
    hello,
    data
  ]);

function required(encoder, r) {
  return Caml_option.some(Curry._1(encoder, r));
}

function optional(encoder, r) {
  if (r !== undefined) {
    return Caml_option.some(Curry._1(encoder, Caml_option.valFromOption(r)));
  }
  
}

function $$null$1(encoder, r) {
  if (r === null) {
    return null;
  } else {
    return Curry._1(encoder, Js_null.getExn(r));
  }
}

function object__(fields) {
  return Json_encode.object_(List.fold_right((function (b, a) {
                    var match = b[1];
                    if (match !== undefined) {
                      return /* :: */Block.simpleVariant("::", [
                                /* tuple */[
                                  b[0],
                                  Caml_option.valFromOption(match)
                                ],
                                a
                              ]);
                    } else {
                      return a;
                    }
                  }), fields, /* [] */0));
}

function activityTimestamps(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "start",
                  optional((function (prim) {
                          return prim;
                        }), r[/* start */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "end",
                      optional((function (prim) {
                              return prim;
                            }), r[/* end_ */1])
                    ],
                    /* [] */0
                  ])
              ]));
}

function activityParty(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "id",
                  optional((function (prim) {
                          return prim;
                        }), r[/* id */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "size",
                      optional((function (param) {
                              return Json_encode.array((function (prim) {
                                            return prim;
                                          }), param);
                            }), r[/* size */1])
                    ],
                    /* [] */0
                  ])
              ]));
}

function activityAssets(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "large_image",
                  optional((function (prim) {
                          return prim;
                        }), r[/* large_image */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "large_text",
                      optional((function (prim) {
                              return prim;
                            }), r[/* large_text */1])
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "small_image",
                          optional((function (prim) {
                                  return prim;
                                }), r[/* small_image */2])
                        ],
                        /* :: */Block.simpleVariant("::", [
                            /* tuple */[
                              "small_text",
                              optional((function (prim) {
                                      return prim;
                                    }), r[/* small_text */3])
                            ],
                            /* [] */0
                          ])
                      ])
                  ])
              ]));
}

function activitySecrets(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "join",
                  optional((function (prim) {
                          return prim;
                        }), r[/* join */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "spectate",
                      optional((function (prim) {
                              return prim;
                            }), r[/* spectate */1])
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "match",
                          optional((function (prim) {
                                  return prim;
                                }), r[/* match */2])
                        ],
                        /* [] */0
                      ])
                  ])
              ]));
}

function activity(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "name",
                  Caml_option.some(r[/* name */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "type",
                      Caml_option.some(r[/* type_ */1])
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "url",
                          optional((function (prim) {
                                  return prim;
                                }), r[/* url */2])
                        ],
                        /* :: */Block.simpleVariant("::", [
                            /* tuple */[
                              "timestamps",
                              optional(activityTimestamps, r[/* timestamps */3])
                            ],
                            /* :: */Block.simpleVariant("::", [
                                /* tuple */[
                                  "application_id",
                                  optional((function (prim) {
                                          return prim;
                                        }), r[/* application_id */4])
                                ],
                                /* :: */Block.simpleVariant("::", [
                                    /* tuple */[
                                      "details",
                                      optional((function (prim) {
                                              return prim;
                                            }), r[/* details */5])
                                    ],
                                    /* :: */Block.simpleVariant("::", [
                                        /* tuple */[
                                          "state",
                                          optional((function (prim) {
                                                  return prim;
                                                }), r[/* state */6])
                                        ],
                                        /* :: */Block.simpleVariant("::", [
                                            /* tuple */[
                                              "party",
                                              optional(activityParty, r[/* party */7])
                                            ],
                                            /* :: */Block.simpleVariant("::", [
                                                /* tuple */[
                                                  "assets",
                                                  optional(activityAssets, r[/* assets */8])
                                                ],
                                                /* :: */Block.simpleVariant("::", [
                                                    /* tuple */[
                                                      "secrets",
                                                      optional(activitySecrets, r[/* secrets */9])
                                                    ],
                                                    /* :: */Block.simpleVariant("::", [
                                                        /* tuple */[
                                                          "instance",
                                                          optional((function (prim) {
                                                                  return prim;
                                                                }), r[/* instance */10])
                                                        ],
                                                        /* :: */Block.simpleVariant("::", [
                                                            /* tuple */[
                                                              "flags",
                                                              optional((function (prim) {
                                                                      return prim;
                                                                    }), r[/* flags */11])
                                                            ],
                                                            /* [] */0
                                                          ])
                                                      ])
                                                  ])
                                              ])
                                          ])
                                      ])
                                  ])
                              ])
                          ])
                      ])
                  ])
              ]));
}

var Presence = /* module */Block.localModule([
    "activityTimestamps",
    "activityParty",
    "activityAssets",
    "activitySecrets",
    "activity"
  ], [
    activityTimestamps,
    activityParty,
    activityAssets,
    activitySecrets,
    activity
  ]);

function updateStatus(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "since",
                  optional((function (prim) {
                          return prim;
                        }), r[/* since */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "game",
                      optional(activity, r[/* game */1])
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "status",
                          Caml_option.some(r[/* status */2])
                        ],
                        /* :: */Block.simpleVariant("::", [
                            /* tuple */[
                              "afk",
                              Caml_option.some(r[/* afk */3])
                            ],
                            /* [] */0
                          ])
                      ])
                  ])
              ]));
}

function heartbeat(param) {
  return $$null$1((function (prim) {
                return prim;
              }), param);
}

function props(r) {
  return Json_encode.object_(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "$os",
                  r[/* os */0]
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "$browser",
                      r[/* browser */1]
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "$device",
                          r[/* device */2]
                        ],
                        /* [] */0
                      ])
                  ])
              ]));
}

function identify(r) {
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "token",
                  Caml_option.some(r[/* token */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "properties",
                      Caml_option.some(props(r[/* properties */1]))
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "compress",
                          optional((function (prim) {
                                  return prim;
                                }), r[/* compress */2])
                        ],
                        /* :: */Block.simpleVariant("::", [
                            /* tuple */[
                              "large_threshold",
                              optional((function (prim) {
                                      return prim;
                                    }), r[/* large_threshold */3])
                            ],
                            /* :: */Block.simpleVariant("::", [
                                /* tuple */[
                                  "shard",
                                  optional((function (param) {
                                          return Json_encode.array((function (prim) {
                                                        return prim;
                                                      }), param);
                                        }), r[/* shard */4])
                                ],
                                /* :: */Block.simpleVariant("::", [
                                    /* tuple */[
                                      "presence",
                                      optional(updateStatus, r[/* presence */5])
                                    ],
                                    /* [] */0
                                  ])
                              ])
                          ])
                      ])
                  ])
              ]));
}

function hello$1(r) {
  return Json_encode.object_(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "heartbeat_interval",
                  r[/* heartbeat_interval */0]
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "_trace",
                      Json_encode.array((function (prim) {
                              return prim;
                            }), r[/* trace */1])
                    ],
                    /* [] */0
                  ])
              ]));
}

function data$1(r) {
  var match = r[/* d */1];
  var tmp;
  if (typeof match === "number") {
    tmp = undefined;
  } else {
    switch (match.tag | 0) {
      case 1 : 
          tmp = Caml_option.some($$null$1((function (prim) {
                      return prim;
                    }), match[0]));
          break;
      case 2 : 
          tmp = Caml_option.some(identify(match[0]));
          break;
      default:
        tmp = undefined;
    }
  }
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "op",
                  Caml_option.some(r[/* op */0])
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "d",
                      tmp
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "s",
                          optional((function (prim) {
                                  return prim;
                                }), r[/* s */2])
                        ],
                        /* :: */Block.simpleVariant("::", [
                            /* tuple */[
                              "t",
                              optional((function (prim) {
                                      return prim;
                                    }), r[/* t */3])
                            ],
                            /* [] */0
                          ])
                      ])
                  ])
              ]));
}

function dataFromPayload(r) {
  var tmp;
  if (typeof r === "number") {
    tmp = undefined;
  } else {
    switch (r.tag | 0) {
      case 1 : 
          tmp = Caml_option.some($$null$1((function (prim) {
                      return prim;
                    }), r[0]));
          break;
      case 2 : 
          tmp = Caml_option.some(identify(r[0]));
          break;
      default:
        tmp = undefined;
    }
  }
  return object__(/* :: */Block.simpleVariant("::", [
                /* tuple */[
                  "op",
                  Caml_option.some(op_of_payload(r))
                ],
                /* :: */Block.simpleVariant("::", [
                    /* tuple */[
                      "d",
                      tmp
                    ],
                    /* :: */Block.simpleVariant("::", [
                        /* tuple */[
                          "t",
                          optional((function (prim) {
                                  return prim;
                                }), t_of_payload(r))
                        ],
                        /* [] */0
                      ])
                  ])
              ]));
}

var Encode = /* module */Block.localModule([
    "required",
    "optional",
    "null",
    "object__",
    "Presence",
    "updateStatus",
    "heartbeat",
    "props",
    "identify",
    "hello",
    "data",
    "dataFromPayload"
  ], [
    required,
    optional,
    $$null$1,
    object__,
    Presence,
    updateStatus,
    heartbeat,
    props,
    identify,
    hello$1,
    data$1,
    dataFromPayload
  ]);

var Presence$1 = 0;

var User$1 = 0;

var Guild$1 = 0;

exports.Presence = Presence$1;
exports.User = User$1;
exports.Guild = Guild$1;
exports.NotImplemented = NotImplemented;
exports.op_of_payload = op_of_payload;
exports.t_of_payload = t_of_payload;
exports.Decode = Decode;
exports.Encode = Encode;
/* No side effect */
